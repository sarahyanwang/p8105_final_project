...
2. What other possible factors are associated with the violation tickets? Does the date associated with the parking violation counts? What about the frequency of vehicle type? 
3. Back to the starting point, what's the suspicious hydrant's distribution that possibly causes the parking violation? How to prevent the violation when parking next to the hydrant the next time?
...
We use the dataset to explored about the distribution of number of of violation and violation types in five boroughs to understand the trend of violation in NYC. Then, we explored the distribution of number of violation over a day and in each weekday and by vehicle types, and fine amounts in boroughs.
...
First, we want to build the mapping for the distribution of parking violation spots.  
...
Then we focus on the distribution of suspicious hydrants distribution that may cause the violation tickets. The basic layer represents the various borough in colors. The red points is the hydrants location which some cars had encountered the violation nearby.

&nbsp;

### Statistical Analyses
We conduct a one-way anova test about the violation frequency to see if there is significant difference between weekdays and month. In addition, we performed a chi-squared test to test the homogeneity of violation types in five boroughs. Next, we performed a proportion test to examine whether the proportion of fire hydrant violation is associated with the population of each borough. Click the link see the details about [Statistical Analyses](statistical-analysis.html). 

&nbsp;

### Discussion

Exploratory analyses foundings:
...
Our statistical analyses findings: 
