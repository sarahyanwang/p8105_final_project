...
#### Fire Hydrants Violation{.tabset}
...
As our primary focus is on fire hydrant violations, we want to do more exploration particularly on fire hydrant violations. We want know how the number of fire hydrant violations distributed cross each borough and over the time.

##### Violations in boroughs
...
hydrant %>%
  ...
  labs(
    title = "Frequency of Violation of parking by fire hydrants",
    x = "borough",
    y = "number of violation"
  ) +
...
hydrant %>%
  ...
  geom_point() + geom_line() + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 5) ) +
  labs(title = "The number of violations of parking by hydrants of each month in 2021") + facet_wrap(~ borough, nrow = 1)
```
...
At first, We are interested in seeing if any conclusions can be made about the frequency at which specific parking violations are given out and if there are any factors that may be associated to receiving one in 2021, as stated in the proposal. Therefore we investigated the boroughs that had the most violations, the borough paid the most in fines and other general topics. 

Further, we narrowed down the range of the topic focus. We did research on the distribution of violations in other variables such as the dataset borough, date, vehicle type, nearby fire hydrants. The most curious factor was the fire hydrants. Were the number of parking violations related to fire hydrants the same across the five boroughs? Following the question, we decided to find the hydrants that seem to be ticketed the most commonly.
...
Our project can make an impact on the community. To be more specific, if merging our findings of fire hydrants locations in NYC to [Google map](https://www.google.com/maps), and generating an alert when parking within the safe range (15 feet) of fire hydrants. It could benefit the drivers to follow the parking regulation and avoid getting fire hydrant violations tickets. 
