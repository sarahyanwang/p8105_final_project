...
violation_type = violation1 %>% 
  ...
  filter(index <= 5)  %>%
  arrange(borough, index)
```

### Frequency Bar Chart of Top 5 Violations by Borough

```{r}
violation_type %>% 
  ggplot(aes(reorder_within(violation, index, borough), y = n, group = borough, fill = violation)) + 
  ...
  labs(
    ...
    y = "Number of Tickets") +
  theme(
    axis.text.x = element_text(angle = 60, vjust = 1.1, hjust = 1, size = 8),
    legend.position = "none"
  ) + facet_grid(.~borough, scales = "free_x") 
