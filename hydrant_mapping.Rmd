...
circles_tbl <-
  tibble(
    unitid = hydrant_to_circle$unitid
  )

for (i in 1:10) {
 
  current_circle <- spCircle(
  10, 
  centerPoint = 
    c(x = hydrant_to_circle$x[[i]], 
      y = hydrant_to_circle$y[[i]])
  )
  circles_tbl$circle[[i]] <- current_circle
}

temp <- list() 
for (i in 1:nrow(circles_tbl)) {
temp[[i]] <- circles_tbl$circle[[i]]$spCircle
}

temp %>% unlist()

SpatialPolygons(temp)
